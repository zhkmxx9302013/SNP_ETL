        ...
        if ($("#increament_mode_date").is(":checked")) {
            for(var i = 0; i < tagCount; i++){
                $("#modal-info-content").append(
                    '<div class="col s12">'+
                    '<div class="card blue col-md-2">'+
                    '<div class="card-content  white-text">'+
                    '<div >' +$("#table_name_list").children().eq(i).text() + '</div>'+
                    '</div>'+
                    '</div>'+
                    '<div class="card blue-grey darken-1 col-md-5">'+
                    '<div class="card-content  white-text">'+
    ...
    $('#commitTask').click(function(){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                var localstroage = window.localStorage;
                localstroage.setItem("taskID", $("#text_TaskID").val());
                localstroage.setItem("taskName", $("#text_Taskname").val());
                localstroage.setItem("tableNameList",JSON.stringify(jsonArr));
                if ($("#is_Increament").is(":checked")) {
                    if ($("#increament_mode_date").is(":checked")) {
                        localstroage.setItem("taskIncreament",'按时间增量');
                    }
                    if ($("#increament_mode_colval").is(":checked")) {
                        localstroage.setItem("taskIncreament",'按列值增量');
                    }
                }else{
                    localstroage.setItem("taskIncreament",'未开启增量');
                }
                localstroage.setItem("tableNameList",JSON.stringify(jsonArr));
